...
Use this only as an optional automation fallback when local assistant project-file processing is not the chosen path, no API key such as `OPENAI_API_KEY`, local command LLM, or precomputed JSONL backend is available, and a local ACP route such as `acpx` can call a Codex-compatible agent.

This workflow is not a corpus source mode. Downloaded PDFs, reviews, replies, and metadata should still be handled as project files through `references/project_corpus_direct_workflow.md`. ACP only supplies the LLM response for each file-based deep-read request.
...
Run from the skill root, or pass paths adjusted for the current workspace:
