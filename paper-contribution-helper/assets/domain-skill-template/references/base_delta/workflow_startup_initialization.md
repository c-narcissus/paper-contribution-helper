...
2. If `knowledge_available=false`, ask for:
   - either a target paper PDF used only for domain inference, or explicit focus domains;
   - years;
   - corpus source and quantity/cap when applicable.
   Prefer an existing project corpus run or downloaded local PDF folder when the user has one. Default assumptions are `source_kind=iclr-openreview` and `domain_scope=computer-science` only when online discovery is actually requested; still require a concrete local corpus source or download target from the user. Do not force the user to name domains when they can upload the paper they want to reframe.
3. Before starting any download, discovery, or PDF standardization, show an initialization confirmation note:
   ...
   - if no focus domains are provided, first read the target paper as a domain seed and infer 1-3 searchable domains/subfields;
...
4. Route by source mode:
   - existing project corpus run: primary route; validate with `contract_project_corpus.md`, then run `initialize_from_project_corpus.py`;
   - local PDF or PDF folder: primary route when raw PDFs are already downloaded; use `workflow_local_pdf_intake.md`;
...
5. Do not paste full PDFs, full extracted texts, or a complete corpus into the assistant context. Keep corpus material as project files, use chunking and intermediate artifacts, and inspect only indexes/manifests/summaries unless debugging one specific paper.
6. If the corpus is present but LLM deep-read is blocked by missing backend, continue project-internal analysis first: validation, PDF extraction, rule pre-pass, per-paper folders, manifest rows, and explicit `not_run` or `failed` LLM status. Do not treat final packaging blockage as a reason to skip project-file analysis.
7. If the user provides an OpenReview/export corpus source, run `scripts/scan_corpus_catalog.py --corpus-source <path>`.
8. Show available fields/years/keywords from `state/corpus_catalog.md`.
9. Run `scripts/configure_initialization.py --corpus-source <path> --focus-domains ... --years ... --source-kind iclr-openreview --domain-scope computer-science`, using either user-supplied domains or the domains inferred from the target paper.
10. Continue with `workflow_knowledge_base_build.md`.
11. Do not mark initialization complete until extraction/synthesis produces non-empty knowledge resources and per-paper analysis coverage.
...
请提供：
1. 目标论文 PDF（只用于推断 1-3 个领域/子领域），或直接给关注领域：
2. 初始化参考年份：
3. 论文来源和数量：
